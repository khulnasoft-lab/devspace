...
from devspace import models, sendchat
from devspace.coders import Coder, base_coder
from devspace.dump import dump  # noqa: F401
from devspace.io import InputOutput

BENCHMARK_DNAME = Path(os.environ.get("DEVSPACE_BENCHMARK_DIR", "tmp.benchmarks"))
...
def main(
    ...
    replay: str = typer.Option(
        ...
        "--replay",
        help=(
            "Replay previous .devspace.khulnasoft.com.history.md responses from previous"
            " benchmark run"
        ),
    ),
    ...
    no_devspace: bool = typer.Option(False, "--no-devspace", help="Do not run devspace"),
    ...
    read_model_settings: str = typer.Option(
        None, "--read-model-settings", help="Load devspace model settings from YAML file"
    ),
    ...
    if "DEVSPACE_DOCKER" not in os.environ:
    ...
    resource_metadata = importlib_resources.files("devspace.resources").joinpath(
        "model-metadata.json"
    )
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.devspace.khulnasoft.com.history.md")
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.devspace.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.devspace.results.json"]
...
def summarize_results(dirname, stats_languages=None):
    ...
    if variants["model"]:
        ...
        command = f"devspace --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:devspace/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".devspace.khulnasoft.com.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".devspace.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".devspace.khulnasoft.com.history.md"
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/devspace/benchmark/npm-test.sh"],
        ".cpp": ["/devspace/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
